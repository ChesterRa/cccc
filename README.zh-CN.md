...
| **多运行时编排** | Claude Code、Codex CLI、ChatGPT Web、Gemini CLI 等 9 种一线运行时可混用，此外还支持 `custom` 运行时兜底 |
...
在支持 Apps/MCP 的 ChatGPT 会话中，**GPT-5.x** 可以参与本地开发，并复用和 Claude Code、Codex 相同的协作层：接收路由消息、通过 CCCC 可见回复、查看和编辑仓库文件、运行受 scope 限制的 shell/git 命令，并和其它 peer agent 协同。当所选 ChatGPT 会话能够看到并调用 CCCC MCP connector 时，Plus/Pro 等付费用户可以获得接近原生 Codex 的本地开发体验。

**GPT-5.x Pro 说明：**受 ChatGPT 平台限制，GPT-5.x Pro 当前无法像支持完整 MCP 的 GPT-5.x 会话一样使用第三方本地读写工具。在 CCCC 中，GPT-5.x Pro 更适合作为高质量的推理、规划和 review 辅助：基于会话上下文、粘贴的 diff、测试输出和项目摘要，提供架构建议、风险审查、调试思路和实现评估；实际本地执行仍由 Codex、Claude Code，或能够访问 CCCC connector 的 ChatGPT Web Model 会话完成。

最短配置路径：

1. 通过公网 HTTPS URL 或 tunnel 暴露 `cccc web`。
2. 在 `Settings > Global > Web Access` 创建 Admin Access Token。
3. 添加 runtime 为 `ChatGPT Web Model` 的 actor。
4. 在 `Settings > Global > ChatGPT Web Model` 为该 actor 创建/复制唯一 MCP URL。
5. 在 ChatGPT Developer mode 中创建 custom MCP connector，填入该 URL，并选择 `No Auth`。
6. 将一个具体的 `https://chatgpt.com/c/...` 对话绑定到该 actor，然后在这个 ChatGPT 对话内选择能够看到并使用 CCCC MCP connector 的 GPT-5.x 模型。
