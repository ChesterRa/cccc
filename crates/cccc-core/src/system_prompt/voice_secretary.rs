...
pub(super) fn is_actor(actor: &Actor) -> bool {
    actor.internal_kind.as_deref().map(str::trim) == Some("voice_secretary")
}

pub(super) fn render(group: &GroupDoc, actor: &Actor) -> String {
    ...
    let mut lines = vec![
        "[CCCC Voice Secretary Runtime Actor]".to_owned(),
        format!("group_id: {}", group.group_id),
        format!("group_title: {title}"),
        format!("actor_id: {actor_id}"),
    ];
    lines.extend(PROMPT_BODY.iter().map(ToString::to_string));
    lines.join("\n").trim().to_owned() + "\n"
}
