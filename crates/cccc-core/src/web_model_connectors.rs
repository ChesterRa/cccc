...
mod tests {
    ...
    fn migrates_settings_array_into_python_connector_file() {
        ...
        let items = load(&home).expect("migrated connectors");
    ...
    fn loads_python_hash_only_connector() {
        ...
        let items = load(&home).expect("load connector");
    ...
    #[test]
    fn actor_retirement_can_be_compensated() {
        let temp = tempfile::tempdir().expect("tempdir");
        let home = HomeLayout::from_path(temp.path().join("home")).expect("home");
        home.initialize().expect("initialize");
        let connector = json!({
            "connector_id":"wmc_actor",
            "group_id":"g_test",
            "actor_id":"web1",
            "secret":"wmcs_actor",
            "created_at":"2026-08-10T00:00:00Z",
        ...
        replace_active(&home, &connector).expect("connector");

        let retired = retire_actor(&home, "g_test", "web1").expect("retire");
        assert_eq!(retired.len(), 1);
        assert_eq!(retired[0]["connector_id"], "wmc_actor");
        assert!(!retired[0]["revoked"].as_bool().unwrap_or(true));
        assert!(secret_matches(&retired[0], "wmcs_actor"));
        assert!(load(&home).expect("retired")[0]["revoked"] == true);

        restore(&home, &retired).expect("restore");
        let restored = load(&home).expect("restored");
        assert!(!restored[0]["revoked"].as_bool().unwrap_or(true));
        assert!(secret_matches(&restored[0], "wmcs_actor"));
    }
}
