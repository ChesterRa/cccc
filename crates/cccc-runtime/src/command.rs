...
pub fn deepseek_external_preflight(
    ...
    if find_executable_in_path(executable, env.get("PATH").map(String::as_str)).is_none() {
...
pub fn detect_runtimes() -> Vec<RuntimeProbe> {
    ...
    .map(|runtime| {
        ...
        let path = find_executable_in_path(&command, discovery_env.get("PATH").map(String::as_str));
...
fn find_executable_in_path(command: &str, path_override: Option<&str>) -> Option<PathBuf> {
    if command.is_empty() {
        return None;
    }
...
mod tests {
    ...
    fn executable_probe_rejects_non_executable_bare_and_absolute_paths() {
        ...
        assert!(super::find_executable_in_path("dsh", Some(&path)).is_none());
        assert!(
            super::find_executable_in_path(dsh.to_str().expect("dsh path is UTF-8"), None)
                .is_none()
        );
    }
