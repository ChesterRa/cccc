...
fn managed_cli_scheduled_update_preserves_live_actor_and_survives_worker_restart() {
    ...
    let root = temp.path();
    cccc_core::fs::atomic_write(&root.join("fixture-marker"), b"controlled-cli-only").unwrap();
    cccc_core::fs::atomic_write(&root.join("target-version"), b"1.0.0").unwrap();
...
"#
        ...
        cccc_core::fs::atomic_write(&path, code.as_bytes()).unwrap();
...
"#,
    ...
    .unwrap();
    std::fs::set_permissions(root.join("mise"), std::fs::Permissions::from_mode(0o700)).unwrap();
    let path = std::env::join_paths([root, Path::new("/usr/bin"), Path::new("/bin")]).unwrap();
    ...
    command
        ...
        .env("PATH", path)
        .env(CHILD_ROOT, root);
    let (mut child, owner) = cccc_runtime::OwnedProcessTree::spawn(&mut command).unwrap();
    ...
    let home = HomeLayout::from_path(root.join("home")).unwrap();
    assert_eq!(
        management::load(&home).unwrap().jobs.len(),
        4,
        "必须实际执行子测试，不能接受零测试通过"
    );
}
...
async fn scheduled_fixture(root: &std::path::Path) {
    ...
    let mut group = store.create("CLI 生命周期验证", "").unwrap();
    ...
    let wait_jobs = |count| async move {
        ...
        loop {
            let state = management::load(job_home).unwrap();
    ...
    let old = super::super::start(&home, &group, &actor("old")).unwrap();
    assert!(old.running);
    let schedule = |id: &str| {
    ...
    cccc_core::fs::atomic_write(&root.join("target-version"), b"2.0.0").unwrap();
    management::save_rules(&home, 0, vec![schedule("update")], chrono::Utc::now()).unwrap();
    ...
    let current = cccc_runtime::status(&group.group_id, "old").unwrap();
    assert!(current.running && current.pid == old.pid);
    super::super::start(&home, &group, &actor("new")).unwrap();
    cccc_runtime::write(&group.group_id, "old", b"still working\r").unwrap();
    ...
    cccc_core::fs::atomic_write(&root.join("target-version"), b"3.0.0").unwrap();
    ...
    assert_eq!(management::load(&home).unwrap().jobs, failed.jobs);
    assert_eq!(
        cccc_runtime::status(&group.group_id, "old").unwrap().pid,
        old.pid
    );
    for id in ["old", "new"] {
        cccc_runtime::stop(&group.group_id, id).unwrap();
    }
    management::submit(
        &home,
        "cursor",
    ...
    super::super::start(&home, &group, &actor("fallback")).unwrap();
    let deadline = Instant::now() + Duration::from_secs(5);
    loop {
    ...
    cccc_runtime::stop(&group.group_id, "fallback").unwrap();
    worker.finish().await;
    println!(
...
fn managed_cli_is_used_for_default_launch_but_explicit_program_is_preserved() {
    ...
    let resolved = resolve_launch_actor(&home, &group, &actor).unwrap();
