...
The Account menu can show the linked member's identity. Account linkage is shown
separately from tunnel connectivity. Temporary directory refresh failures retain
navigation with an explicit status while authorization expiry still blocks access.
Group rows show explicit cross-member connection counts; the account website
separates pending, active and historical relations and offers a return to your Group.
