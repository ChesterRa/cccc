...
- 当前基线原生 Runtime 清单为 19 项，已知安装来源候选为 17 项。CLI 管理页应按 CCCC 服务端操作系统与架构筛选可受管安装的 CLI，排除 `web_model` 与 `custom`；不能把某平台不支持受管安装的 Runtime 从通用目录或 Actor 配置删除。平台筛选是 2026-09-11 新确认要求，尚未实现或验收，不以当前 Linux 17 项结果推导 Windows 支持。
...
- 上述“不覆盖”约束 CLI 管理新增选择。DeepSeek 的原生 `launch_command::resolve` 本就按 `dsh/dsh-acp-demo` 文件名重写命令；本补丁不修订该旧行为。显式命令的 setup 不读取 CLI 管理状态或选择受管环境，保留原生 setup/resolve；默认命令及 `cccc setup` 仍验证受管记录。原生同名显式路径优先级问题需独立授权与评审，不能宣称本功能已修复它。
...
- 使用锁仅在识别受管路径归属时读取管理记录；外部命令不因管理 JSON 损坏而失败，默认受管选择的严格校验仍由启动入口负责。现有路径 canonicalize 后匹配版本目录，覆盖 `../` 和目录外的符号链接别名；已声明的受管路径缺失或归属未知时拒绝启动。卸载始终锁所属 Runtime，目录仍由原安装/更新操作拥有，卸载操作本身不生成版本目录。
...
- 有附加资源的 PTY/DeepSeek 在读写器初始化失败时执行原生终止/回收路径；PTY 只在确认 wait 成功后释放，DeepSeek 复用 stop 的有界轮询。无资源的原生失败路径不改。PTY 停止前为资源接入路径补充退出检查；停止失败仍不伪装成功。AnalystSession 有主进程且所有主/辅助进程确认退出后，在状态轮询中释放 CLI 使用锁；协议断开或进程查询错误不能充当退出证据。
...
写入中的末尾 JSONL 可能尚未完整，此时 `has_more=true` 不保证游标前进；Web 只有在 `next_offset > 当前 offset` 时启用下一页，仍继续刷新当前页，避免重复压入相同分页位置。
...
- CLI 计划 ID 沿用相同字符合同，允许下划线或连字符开头；编辑器与后端一致，保存、重新编辑及删除旧计划不要求迁移。不修改工作组自动化的共享 ID 校验器。
...
最终放行必须完成 [最终检查与跨平台验收](cli-management-final-checks.md)，包括当前 GitHub CI、Windows 原生回归、逐 CLI 平台支持以及 Windows 真实 GUI 验收。下方及 Web 文档中的历史通过数不替代新门禁；本次仅扩充规格与用例，尚未执行新增验证。

| 范围 | 必须验证 |
|---|---|
| 来源与安装 | 17 项候选逐项核对平台、架构、mise 后端及供应商发行包；各平台可安装子集的隔离安装/更新、同版本修复及失败保留；不支持项隐藏且后端阻止执行 |
...
| Web | Linux 与 Windows 真实后端分别验收；原生风格与三语、平台可安装清单、中文术语、操作记录、日志、计划交互及故障恢复；浏览器系统不决定服务器支持清单 |
| Windows 兼容 | 原生二进制构建及启动不受影响；保留原有 CLI 探测与 Actor 功能；原有 windows-smoke 全部通过，不以交叉编译或跳过测试替代 |
