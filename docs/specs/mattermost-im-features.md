...
2026-09-14 review17：F27 的纯旧平台管理续体跨组/关窗问题可能是上游既有 Bug，用户明确决定本次不修复；[范围与证据说明](mattermost-im.md#review17-范围裁定旧平台管理续体不修复)为唯一处置合同。此排除不撤销 Mattermost 的既有保护；F02 的[配置修订记录回收](mattermost-im.md#配置修订记录回收review17)已补实现和回归，实际验证结果见验收，不沿用旧通过记录。

2026-09-14 review16：F02/F27 补安装后状态失败回收、管理错误可见及微信专用入口参与排序，见[当前合同](mattermost-im.md#当前修复合同2026-09-14review16)；两平台适用检查及实际 GUI 已通过，失败与反证记录见验收。

2026-09-14 review15：F02/F27 补 daemon 协议配置委托、管理请求期间的草稿和同浏览器同组请求顺序，见[历史合同](mattermost-im.md#历史修复合同2026-09-14review15)；两平台适用全套及实际 Web/worker 回归通过，普通 CLI 已有的 Web 入口不变。

2026-09-14 review14：F27 补初次读取期间用户选择/编辑的归属，F29 澄清出站 MIME 由目标平台分类，见[历史合同](mattermost-im.md#历史修复合同2026-09-14review14)。

2026-09-14 review13：F02 的手动/自动恢复启动前请求版本、F27 的同组切换平台访问归属，见[历史合同](mattermost-im.md#历史修复合同2026-09-14review13)。只补受影响入口，不将既有两旧平台间的异步行为纳入本 PR。

2026-09-14 review12：F02/F26 的旧管理动作与同配置请求隔离、F01/F27 的地址错误、F32 的公共 CLI 导航，以及 Web 跨 Group 异步管理结果的归属要求，统一见[历史合同](mattermost-im.md#历史修复合同2026-09-14review12)和[当轮验收](mattermost-im-acceptance.md#旧管理请求与异步表单交接2026-09-14review12)。历史验收不覆盖新增交接窗口。

2026-09-14 review11：F02 补启动代次分配前的恢复停止及跨平台切换窗口；F15/F16 补长流式成功终态完整分段与最终正文去重。合同见[review11](mattermost-im.md#历史修复合同2026-09-14review11)，执行状态见[对应验收](mattermost-im-acceptance.md#启动前窗口与长流式终态2026-09-14review11)。只有全部终态分段成功才去重，部分失败仍保留完整最终兜底。
...
2026-09-13 规范复核修订：F21/F22/F25 补提交结果不明的安全提示、反应清理及日志边界；F04/F21 补早期查询失败的授权反馈；F24 补运行期 Ping/Pong 写入截止时间。下表旧验收数字属于历史，新用例及随后通过的 V01 真实业务补收/恢复前撤权验收单列于[验收记录](mattermost-im-acceptance.md)，不以模拟 daemon 或 Bot 自发协议帖替代人类到 Actor 的实际链路。
...
| F02 | 组级 start/stop、任务回收、恢复 enabled；[C1][C5] | 同一组级生命周期管理 WS/外发任务，不控制 Actor | 既有范围保留；跨平台回写/替换见 review8/9；daemon 协议委托见 review15；安装后状态失败的回收见 review16（两平台通过） | 历史 T04；MM-CROSS-START；review9/15/16 |
...
| F17 | 入站文件下载、体积检查、Blob 存储及元数据；[C16][C17] | GET 文件及元数据，核对帖子归属；同帖全部下载/验证后保存，失败清理未保存内容，不信任任意远端 URL；[M5] | 本轮两平台回归通过；历史 T13/T14 与多附件受控回归分开记录 | T13、T14；2026-09-14 多附件修订 |
...
| F20 | client_id 交由 daemon 去重；部分平台另有有界内存去重；[C10][C20] | 同一 MM post_id 重复事件使用同一请求标识；不增加新持久队列 | 既有入账去重证据保留；失败反馈去重已修复，当前回归结果见 review8 | 历史 T15；MM-LOOKUP-REPLAY |
...
|---|---|---|---|---|
| F27 | Token 字面量/环境变量引用/旧别名规范化，保存既有 files 策略；[C2][C22] | 复用同一规则，额外保存 MM 站点；真实文件上限以执行路径为准，不能只测配置保存 | 原规范化已接入；管理排序/草稿保护见 review15；失败可见及微信入口补齐见 review16（两平台通过） | 历史 T02、T14；review15/16 |
...
| F29 | Telegram/微信/WeCom 接收图片、音频、视频等并转换通用媒体元数据；[C17][C23][C24] | MM 对文件类媒体使用 file_ids 下载/上传；入站保留平台 MIME，出站保留文件名/字节并由平台分类，不保证目标 MIME 与附件元数据逐字相同；不承诺语音识别或原播放器外观 | 代码已接入；T13 为历史证据，review14 仅补固定服务端分类依据，不新增真实媒体联调声明 | T13 |
