...
2026-09-14 review15：F02/F27 补 daemon 协议配置委托、管理请求期间的草稿和同浏览器同组请求顺序，见[当前合同](mattermost-im.md#当前修复合同2026-09-14review15)；两平台适用全套及实际 Web/worker 回归通过，普通 CLI 已有的 Web 入口不变。
...
| F02 | 组级 start/stop、任务回收、恢复 enabled；[C1][C5] | 同一组级生命周期管理 WS/外发任务，不控制 Actor | 既有范围保留；跨平台回写/替换见 review8/9；daemon 协议委托与实际 worker 关闭见 review15 | 历史 T04；MM-CROSS-START；review9/15 |
...
|---|---|---|---|---|
| F27 | Token 字面量/环境变量引用/旧别名规范化，保存既有 files 策略；[C2][C22] | 复用同一规则，额外保存 MM 站点；真实文件上限以执行路径为准，不能只测配置保存 | 原规范化已接入；新增在途管理排序/草稿保护见 review15，不由历史结果代替 | 历史 T02、T14；review15 |
