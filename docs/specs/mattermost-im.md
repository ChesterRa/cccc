...
本轮补充（2026-09-13，PR #103 第二轮审核）：沿用企业微信连接器的有界入站队列和独立 worker，Mattermost 的附件处理不再占用 WebSocket 心跳循环。队列满时施加读取背压，不丢弃已接收事件，不无限创建任务；背压期间继续发送心跳，恢复读取后重新计算接收超时。入站按接收顺序处理，停止连接器时两个任务一起取消。保持原有断线重连及不自动补拉历史的合同。

处理反应仍复用原生 `Active` 和清理机制，仅在 Mattermost 的 daemon 提交及事件 ID 绑定之间加异步互斥；最终反应等待绑定后再匹配，锁不覆盖附件下载或反应 HTTP。不修改其他连接器的反应语义，不引入完成事件缓存。

Mattermost 平台草稿增加工作组来源校验，同组平台切换仍恢复未保存内容，切换工作组后不恢复另一组的地址或 Token；其他平台草稿机制不变。补齐既有配置测试夹具的必需字段，不放宽生产类型。新增回归须验证慢附件期间心跳、队列背压与顺序、完成早于绑定及重复/无关完成、保持挂载时的跨组草稿隔离。术语沿用工作组（Group）、聊天连接器（IM Connector）、附件和协作事件（Event），不增加领域概念；执行结果另记验收记录。
...
| 位置 | 仅允许的必要变化 |
|---|---|
| [im_runtime.rs](../../crates/cccc-web/src/im_runtime.rs) | 模块声明及 start 分派 |
...
- 复用原生重连、内存去重及 ledger lag 补读，明确跨重启历史不自动回放的既有合同。不新增后台积压任务重放、持久待发箱或 exactly-once 承诺。
