...
The Rust and Python daemons do not parse PTY output to infer activity for eligible provider sessions. Codex PTY activity comes from lifecycle hooks injected only into processes that CCCC starts: prompt and tool events report `working`, permission requests report `waiting`, and verified stop/session events report `idle` or `stopped`. CCCC registers both `StopFailure` and `PostToolUseFailure`: they publish a short-lived `failed` runtime-activity outcome, while a failed completed turn leaves the still-running actor `idle` and ready for another turn. The hook does not retain provider error text, so `StopFailure` identifies a generic turn failure rather than a capacity-specific cause. Every injected hook process carries a per-launch fence, and Codex turn-scoped events must identify the active provider turn or its bound tool operation. Tool operations are observed serially: a second operation cannot start before the active one closes, and operation-specific events must carry the exact active operation ID. Late events from an older launch, session, turn, or operation cannot overwrite current state.
