...
const CONTROL_ACTIVE_CLASS: Record<GroupControlKey, string> = {
  ...
  stop: "bg-rose-700 text-white shadow-[0_10px_24px_rgba(190,24,93,0.28)] ring-1 ring-black/5 dark:ring-white/10",
};
...
/** Run actions a group's sidebar menu offers: one launch/resume/pause toggle, plus stop unless already stopped. */
export function getGroupMenuControls(
  ...
  const toggle: GroupMenuControl =
    status === "run"
      ...
      : getLaunchControlMode(status) === "activate"
        ? { control: "activate", labelKey: "resumeDelivery" }
        : { control: "launch", labelKey: "launchAllAgents" };
  return status === "stop" ? [toggle] : [toggle, { control: "stop", labelKey: "stopAllAgents" }];
}

export function getLaunchControlMode(
  status: GroupStatusKey | null | undefined,
): "start" | "activate" {
...
export function getGroupControlVisual(
  ...
  const active = Boolean(
    status &&
    (CONTROL_ACTIVE_STATUS[control] === status || (control === "launch" && status === "idle")),
  );
