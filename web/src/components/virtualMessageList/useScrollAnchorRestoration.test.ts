...
describe("getScrollRestorationRequestKey", () => {
  it("distinguishes a late anchor handoff from an initial bottom fallback", () => {
    ...
    expect(
      getScrollRestorationRequestKey({ anchorId: "event-1", offsetPx: 24 }),
    ).toBe("anchor:event-1:24");
  });

  it("rounds sub-pixel measurement noise without hiding meaningful movement", () => {
    expect(
      getScrollRestorationRequestKey({ anchorId: "event-1", offsetPx: 24.12 }),
    ).toBe("anchor:event-1:24");
    expect(
      getScrollRestorationRequestKey({ anchorId: "event-1", offsetPx: 25 }),
    ).toBe("anchor:event-1:25");
  });
...
describe("scroll anchor correction lifecycle", () => {
  it("keeps checking for virtual measurement drift during the restoration window", () => {
    const source = readFileSync(new URL("./useScrollAnchorRestoration.ts", import.meta.url), "utf8");
