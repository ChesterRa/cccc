...
describe("federation settings placement", () => {
  ...
  it("libp2p connection info normalizes issuer endpoint and surfaces local crypto errors before invite creation", () => {
    const src = readSource("./FederationLibp2pPairingSection.tsx");
    expect(src).toContain("normalizeIssuerEndpoint(issuerEndpoint)");
    expect(src.indexOf("assertSha256Available();")).toBeLessThan(src.indexOf("createFederationPairingInvite"));
    expect(src).toContain('t("federation.sha256Unavailable")');
  ...
  it("libp2p connection info includes the current group title for remote display", () => {
    const src = readSource("./FederationLibp2pPairingSection.tsx");
    expect(src).toContain("groupTitle");
    expect(src).toContain("groupTitle: currentGroupTitle || currentGroupId");
  });
