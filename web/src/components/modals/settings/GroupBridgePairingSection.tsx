...
export function GroupBridgePairingSection({
  ...
  const overview = useMemo(
    () => projectPairingOverview({ identity, requests, trusts }),
    [identity, requests, trusts],
  );
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (!infoResp.ok) {
          ...
          setInviteError(
            errorKey ? t(errorKey) : infoResp.error.message || t("group_bridge.createInviteFailed"),
          );
          return;
        }
  ...
  const copyTrustRecipientIdentifier = useCallback(
    async (trust: GroupBridgeTrust, displayName: string, accessLevel: string) => {
      ...
      setTrustCopyNotice(
        copied
          ? t("group_bridge.copyRecipientIdentifierDone", {
              defaultValue: "Recipient identifier copied.",
            })
          : t("group_bridge.copyRecipientIdentifierManual", {
              defaultValue: "Copy is unavailable; select the recipient identifier manually.",
            }),
      );
    },
    [t],
  ...
  const onCreateRequest = useCallback(async () => {
    ...
    try {
      const resp =
        parsed.isRemote && parsed.payload
      ...
      if (resp.ok) {
        ...
        setRequestError(
          errorKey
            ? t(errorKey)
            : shouldUsePairingCodeHelp(resp.error.message)
              ? t("group_bridge.pairingCodeInvalidOrExpired")
              : resp.error.message || t("group_bridge.submitRequestFailed"),
        );
      }
    } finally {
      setBusy(false);
    ...
    async (requestId: string, action: "approve" | "reject") => {
      ...
      try {
        ...
        if (!resp.ok)
          setReviewError(
            resp.error.message ||
              t(action === "approve" ? "group_bridge.approveFailed" : "group_bridge.rejectFailed"),
          );
        await refreshPairing();
  ...
  const revokeTrustedPeer = useCallback(
  ...
  const updateTrustAccess = useCallback(
    async (trustId: string, accessLevel: GroupBridgeAccessLevel) => {
      const tid = String(trustId || "").trim();
      if (!tid) return;
      setReviewError("");
  ...
  const refreshTrustRemoteInfo = useCallback(
    async (trustId: string) => {
      const tid = String(trustId || "").trim();
      if (!tid) return;
      setReviewError("");
      ...
      try {
        ...
        if (!resp.ok) {
          setRemoteRefreshErrors((prev) => ({
            ...
            [tid]:
              resp.error.message ||
              t("group_bridge.refreshRemoteInfoFailed", {
                defaultValue: "Could not refresh remote info.",
              }),
          }));
          return;
        }
  ...
  const deleteOutbound = useCallback(
    async (outboundId: string) => {
      ...
      try {
        ...
        if (!resp.ok)
          setReviewError(resp.error.message || t("group_bridge.deleteSentRequestFailed"));
        await refreshPairing();
      } finally {
  ...
  const trustedRemoteGroupsSection = (
    <section className={settingsWorkspacePanelClass(isDark)}>
      <div className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
        {t("group_bridge.trustedRemoteGroups")}
      </div>
      <p className="mt-2 text-xs leading-5 text-[var(--color-text-muted)]">
        {t("group_bridge.trustedRemoteGroupsHelp")}
      </p>
      {trustCopyNotice && (
      ...
      {trustedPeers.length === 0 ? (
        <p className="mt-3 text-xs text-[var(--color-text-muted)]">{t("group_bridge.noneYet")}</p>
      ) : (
        <div className="mt-3 space-y-3">
          {trustedPeers.map((trust) => {
            const remoteGroupLabel = formatRemoteGroupLabel(
              trust,
              t("group_bridge.unknownRemoteGroup", { defaultValue: "Unknown remote group" }),
            );
            const remoteGroupId = String(trust.remote_group_id || "").trim();
            ...
            return (
              <div
                key={trust.trust_id}
                className="rounded-2xl border border-[var(--glass-border-subtle)] bg-[var(--color-bg-secondary)] px-4 py-3"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-[var(--color-text-muted)]">
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 font-medium text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-200">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    {remoteGroupId && (
                      <button
                        ...
                        onClick={() =>
                          copyTrustRecipientIdentifier(
                            trust,
                            remoteGroupLabel,
                            remoteAccessKnown ? remoteAccessLevel : "unknown",
                          )
                        }
                      >
                        {t("group_bridge.copyRecipientIdentifier", {
                    ...
                    >
                      {revokeBusyId === trust.trust_id
                        ? t("group_bridge.revokingTrust")
                        : t("group_bridge.revokeTrust")}
                    </button>
                  </div>
                </div>
                ...
                <div className="mt-3 grid gap-3 xl:grid-cols-2">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-[180px] flex-1">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
                          {t("group_bridge.remoteAccessToThisGroup")}
                        </div>
                        <p className="mt-1 text-xs leading-5 text-[var(--color-text-muted)]">
                          {t("group_bridge.remoteAccessToThisGroupHelp")}
                        </p>
                      </div>
                  ...
                  <div className="rounded-xl border border-[var(--glass-border-subtle)] bg-[var(--glass-panel-bg)] px-3 py-2.5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-[180px] flex-1">
                        <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
                          {t("group_bridge.thisGroupAccessToRemote")}
                        </div>
                        <p className="mt-1 text-xs leading-5 text-[var(--color-text-muted)]">
                          {t("group_bridge.thisGroupAccessToRemoteHelp")}
                        </p>
                      </div>
                      <button
                        type="button"
                      ...
                      >
                        <RefreshIcon
                          size={14}
                          className={remoteRefreshBusy ? "animate-spin" : ""}
                        />
                        {remoteRefreshBusy
                          ? t("group_bridge.refreshingConnections")
                          : t("group_bridge.refreshRemoteInfo")}
                      </button>
                    </div>
                    ...
                    <p className="mt-2 text-xs leading-5 text-[var(--color-text-muted)]">
                      {t(
                        `group_bridge.remoteAccessDescriptions.${remoteAccessKnown ? remoteAccessLevel : "unknown"}`,
                      )}
                    </p>
                    {refreshError && (
                ...
                <p className="mt-3 text-xs leading-5 text-[var(--color-text-muted)]">
                  {t("group_bridge.runtimeRestartHint")}
                </p>
              </div>
            );
          })}
    ...
    <div className={settingsWorkspaceBodyClass}>
      <section className={settingsWorkspacePanelClass(isDark)}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
              {t("group_bridge.overview")}
            </div>
            <h3 className="mt-1 text-lg font-semibold text-[var(--color-text-primary)]">
              {t("group_bridge.connectRemoteCcccGroup")}
            </h3>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-text-muted)]">
              {t("group_bridge.groupConnectionsLead", {
                group: currentGroupTitle || currentGroupId || "-",
              })}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-[var(--glass-border-subtle)] bg-[var(--color-bg-secondary)] px-3 py-1 text-xs font-medium text-[var(--color-text-primary)]">
              {overview.identityReady
                ? t("group_bridge.identityReady")
                : t("group_bridge.identityPending")}
            </span>
            <span className="rounded-full border border-amber-300/40 bg-amber-100/70 px-3 py-1 text-xs font-medium text-amber-800 dark:bg-amber-500/15 dark:text-amber-200">
              {t("group_bridge.pendingCount", { count: overview.pendingCount })}
            </span>
            <span className="rounded-full border border-emerald-300/40 bg-emerald-100/70 px-3 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-200">
              {t("group_bridge.trustedCount", { count: overview.trustedCount })}
            </span>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className={settingsWorkspacePanelClass(isDark)}>
          <div className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
            {t("group_bridge.createConnectionInfo")}
          </div>
          <p className="mt-2 text-sm leading-6 text-[var(--color-text-muted)]">
            {t("group_bridge.createConnectionInfoHelp")}
          </p>
          <label
            className="mt-4 block text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]"
            htmlFor="group_bridge-issuer-endpoint"
          >
            {t("group_bridge.issuerEndpoint")}
          </label>
          <input
          ...
          <button
            type="button"
            className={`mt-4 ${primaryButtonClass(busy)}`}
          ...
          {createdInfo && (
            <div className="mt-4 rounded-2xl border border-emerald-300/40 bg-emerald-50/80 p-4 text-sm text-emerald-900 dark:bg-emerald-500/10 dark:text-emerald-100">
              <div className="text-xs font-semibold uppercase tracking-wide">
                {t("group_bridge.oneTimeConnectionInfo")}
              </div>
              <pre className="mt-2 max-h-48 overflow-auto rounded-lg bg-white/70 p-3 font-mono text-xs text-emerald-950 dark:bg-black/20 dark:text-emerald-50">
                {createdInfo}
              </pre>
              <button
                type="button"
                className={`mt-3 ${secondaryButtonClass("sm")}`}
                onClick={onCopyConnectionInfo}
              >
                {t("group_bridge.copyConnectionInfo")}
              </button>
              <p className="mt-2 text-xs leading-5">
                {t("group_bridge.oneTimeConnectionInfoHelp")}
              </p>
        ...
        <section className={settingsWorkspacePanelClass(isDark)}>
          <div className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
            {t("group_bridge.pasteConnectionInfo")}
          </div>
          <p className="mt-2 text-sm leading-6 text-[var(--color-text-muted)]">
            {t("group_bridge.pasteConnectionInfoHelp")}
          </p>
          <textarea
            className={`${inputClass(isDark)} mt-4 min-h-[118px] resize-y font-mono text-xs`}
            value={connectionInput}
            onChange={(event) => setConnectionInput(event.target.value)}
            placeholder={t("group_bridge.remotePayloadPlaceholder")}
          />
          <p className="mt-3 text-xs text-[var(--color-text-muted)]">
            {t("group_bridge.localPeerAutoMetadata")}
          </p>
          {parsed.isRemote && parsed.issuerEndpoint && (
            <div className="mt-3 rounded-2xl border border-[var(--glass-border-subtle)] bg-[var(--color-bg-secondary)] px-4 py-3 text-xs leading-5 text-[var(--color-text-muted)]">
              <div className="font-semibold text-[var(--color-text-primary)]">
                {t("group_bridge.remotePayloadDetected")}
              </div>
              <div className="mt-1">
                {t("group_bridge.remotePayloadTarget", {
                  endpoint: parsed.issuerEndpoint,
                  group: parsed.remoteGroupId || t("group_bridge.unknownPeer"),
                })}
              </div>
            </div>
          )}
          ...
          <button
            type="button"
            className={`mt-4 ${secondaryButtonClass("md")}`}
            disabled={!requestReady}
            onClick={onCreateRequest}
          >
            {t("group_bridge.submitRequest")}
          </button>
        </section>
      </div>
      ...
      <section className={settingsWorkspacePanelClass(isDark)}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
            {t("group_bridge.incomingRequests")}
          </div>
          <button
            type="button"
            className={secondaryButtonClass("sm")}
          ...
          >
            ...
            {refreshBusy
              ? t("group_bridge.refreshingConnections")
              : t("group_bridge.refreshConnections")}
          </button>
        </div>
        {reviewError && (
          <p className="mt-3 text-xs font-medium text-rose-600 dark:text-rose-400">{reviewError}</p>
        )}
        {incomingRequests.length === 0 ? (
          <p className="mt-3 text-xs text-[var(--color-text-muted)]">
            {t("group_bridge.nonePending")}
          </p>
        ) : (
          <div className="mt-3 space-y-2">
            {incomingRequests.map((request) => (
              <div
                ...
                className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--glass-border-subtle)] bg-[var(--color-bg-secondary)] px-4 py-3"
              >
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-[var(--color-text-primary)]">
                    {formatPeerLabel(request, t("group_bridge.unknownPeer"))}
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-[var(--color-text-muted)]">
                    <span className="rounded-full bg-amber-100 px-2 py-0.5 font-medium text-amber-800 dark:bg-amber-500/15 dark:text-amber-200">
                      {t("group_bridge.pendingBadge")}
                    </span>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={secondaryButtonClass("sm")}
                    disabled={busy}
                    onClick={() => review(request.request_id, "approve")}
                  >
                    {t("group_bridge.approve")}
                  </button>
                  <button
                    type="button"
                    className={dangerButtonClass("sm")}
                    disabled={busy}
                    onClick={() => review(request.request_id, "reject")}
                  >
                    {t("group_bridge.reject")}
                  </button>
                </div>
              </div>
            ))}
          ...
          <div className="mt-3 space-y-2">
            {recentOutbounds.map((outbound) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-[var(--color-text-muted)]">
                    <span className="rounded-full bg-sky-100 px-2 py-0.5 font-medium text-sky-800 dark:bg-sky-500/15 dark:text-sky-200">
                      {t("group_bridge.status", { status: outbound.status })}
                    </span>
                ...
                >
                  {deleteOutboundBusyId === outbound.outbound_id
                    ? t("group_bridge.deletingSentRequest")
                    : t("group_bridge.deleteSentRequest")}
                </button>
              </div>
            ))}
          </div>
      ...
      <details className={settingsWorkspacePanelClass(isDark)}>
        <summary className="cursor-pointer text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
          {t("group_bridge.localDiagnostics")}
        </summary>
        <p className="mt-3 text-xs text-[var(--color-text-muted)]">
          {t("group_bridge.localDiagnosticsHelp")}
        </p>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div className={settingsWorkspaceSoftPanelClass(isDark)}>
            <div className="text-[11px] font-semibold uppercase text-[var(--color-text-muted)]">
              {t("group_bridge.nodeId")}
            </div>
            <div className="mt-1 break-all text-sm text-[var(--color-text-primary)]">
              {localNodeId || t("group_bridge.notLoaded")}
            </div>
          </div>
          <div className={settingsWorkspaceSoftPanelClass(isDark)}>
            <div className="text-[11px] font-semibold uppercase text-[var(--color-text-muted)]">
              {t("group_bridge.peerId")}
            </div>
            <div className="mt-1 break-all text-sm text-[var(--color-text-primary)]">
              {localPeerId || t("group_bridge.notLoaded")}
            </div>
          </div>
          <div className={`${settingsWorkspaceSoftPanelClass(isDark)} sm:col-span-2`}>
            <div className="text-[11px] font-semibold uppercase text-[var(--color-text-muted)]">
              {t("group_bridge.sessionTransport")}
            </div>
            <div className="mt-1 break-all text-sm text-[var(--color-text-primary)]">
              {t("group_bridge.sessionTransportManaged")}
            </div>
          </div>
        </div>
      </details>
