...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_actor_add.add_argument("--profile-id", default="", help="Link this actor to a reusable profile")
    p_actor_add.add_argument("--profile-scope", choices=["global", "user"], default="global", help="Profile scope for --profile-id (default: global)")
    p_actor_add.add_argument("--profile-owner-id", default="", help="Profile owner id for user-scoped profiles")
    ...
    p_actor_update.add_argument("--profile-id", default="", help="Attach this actor to a reusable profile")
    p_actor_update.add_argument("--profile-scope", choices=["global", "user"], default="global", help="Profile scope for --profile-id (default: global)")
    p_actor_update.add_argument("--profile-owner-id", default="", help="Profile owner id for user-scoped profiles")
    p_actor_update.add_argument("--profile-action", choices=["convert_to_custom"], default=None, help="Profile linkage action")
    ...
    p_actor_profile = actor_sub.add_parser("profile", help="Manage reusable actor runtime profiles")
    actor_profile_sub = p_actor_profile.add_subparsers(dest="profile_action", required=True)

    p_actor_profile_list = actor_profile_sub.add_parser("list", help="List reusable actor profiles")
    p_actor_profile_list.add_argument("--view", choices=["global", "my", "all"], default="global", help="Profile view (default: global)")
    p_actor_profile_list.add_argument("--by", default="user", help="Requester (default: user)")
    p_actor_profile_list.set_defaults(func=cmd_actor_profile_list)

    p_actor_profile_get = actor_profile_sub.add_parser("get", help="Show one reusable actor profile")
    p_actor_profile_get.add_argument("profile_id", help="Profile id")
    p_actor_profile_get.add_argument("--scope", choices=["global", "user"], default="global", help="Profile scope (default: global)")
    p_actor_profile_get.add_argument("--owner-id", default="", help="Profile owner id for user-scoped profiles")
    p_actor_profile_get.add_argument("--by", default="user", help="Requester (default: user)")
    p_actor_profile_get.set_defaults(func=cmd_actor_profile_get)

    p_actor_profile_upsert = actor_profile_sub.add_parser("upsert", help="Create or update a reusable actor profile")
    p_actor_profile_upsert.add_argument("--id", dest="profile_id", default="", help="Profile id (optional for create)")
    p_actor_profile_upsert.add_argument("--name", default=None, help="Profile display name")
    p_actor_profile_upsert.add_argument(
        "--runtime",
        choices=["claude", "codex", "droid", "amp", "auggie", "neovate", "gemini", "kimi", "custom"],
        default=None,
        help="Profile runtime",
    )
    p_actor_profile_upsert.add_argument("--runner", choices=["pty", "headless"], default=None, help="Profile runner")
    p_actor_profile_upsert.add_argument("--command", default=None, help="Command to store for this profile")
    p_actor_profile_upsert.add_argument("--submit", choices=["enter", "newline", "none"], default=None, help="Submit key")
    p_actor_profile_upsert.add_argument("--scope", choices=["global", "user"], default=None, help="Profile scope")
    p_actor_profile_upsert.add_argument("--owner-id", default=None, help="Profile owner id for user-scoped profiles")
    p_actor_profile_upsert.add_argument("--expected-revision", type=int, default=None, help="Expected profile revision for optimistic writes")
    p_actor_profile_upsert.add_argument("--capability-defaults", default="", help="JSON object for capability defaults")
    p_actor_profile_upsert.add_argument("--by", default="user", help="Requester (default: user)")
    p_actor_profile_upsert.set_defaults(func=cmd_actor_profile_upsert)

    p_actor_profile_delete = actor_profile_sub.add_parser("delete", help="Delete a reusable actor profile")
    p_actor_profile_delete.add_argument("profile_id", help="Profile id")
    p_actor_profile_delete.add_argument("--scope", choices=["global", "user"], default="global", help="Profile scope (default: global)")
    p_actor_profile_delete.add_argument("--owner-id", default="", help="Profile owner id for user-scoped profiles")
    p_actor_profile_delete.add_argument("--force-detach", action="store_true", help="Detach linked actors before delete when supported")
    p_actor_profile_delete.add_argument("--by", default="user", help="Requester (default: user)")
    p_actor_profile_delete.set_defaults(func=cmd_actor_profile_delete)

    p_actor_profile_secrets = actor_profile_sub.add_parser("secrets", help="Manage runtime-only secrets for a reusable actor profile")
    p_actor_profile_secrets.add_argument("profile_id", help="Profile id")
    p_actor_profile_secrets.add_argument("--scope", choices=["global", "user"], default="global", help="Profile scope (default: global)")
    p_actor_profile_secrets.add_argument("--owner-id", default="", help="Profile owner id for user-scoped profiles")
    p_actor_profile_secrets.add_argument("--set", action="append", default=[], help="Set secret env (KEY=VALUE), repeatable")
    p_actor_profile_secrets.add_argument("--unset", action="append", default=[], help="Unset secret key (KEY), repeatable")
    p_actor_profile_secrets.add_argument("--clear", action="store_true", help="Clear all secrets for this profile")
    p_actor_profile_secrets.add_argument("--keys", action="store_true", help="List configured keys (no values)")
    p_actor_profile_secrets.add_argument("--by", default="user", help="Requester (default: user)")
    p_actor_profile_secrets.set_defaults(func=cmd_actor_profile_secrets)
