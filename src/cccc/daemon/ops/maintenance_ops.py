...
def handle_send_cross_group(
    ...
    *,
    ...
    if message_mode not in {"send", "request_reply", "mail"}:
        return _error("invalid_message_mode", "message_mode is required and must be send, request_reply, or mail")
    ...
    if remote_route is not None:
        ...
        try:
            validate_message_audience(dst_to_tokens, message_mode=message_mode)
        except PeerRecipientError as exc:
            return _error(exc.code, exc.message, details=exc.details)
        if message_mode == "request_reply" and any(
            recipient in {"@all", "@peers", "@foreman"} for recipient in dst_to_tokens
        ):
            return _error(
