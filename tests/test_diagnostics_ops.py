...
class TestDiagnosticsOps(unittest.TestCase):
    ...
    def test_terminal_snapshot_renders_screen_without_raw_ansi(self) -> None:
        from unittest.mock import patch
        ...
        try:
            ...
            self.assertTrue(resp.ok, getattr(resp, "error", None))
            self.assertEqual((resp.result or {}).get("text"), "current")
            self.assertEqual((resp.result or {}).get("end_cursor"), 25)
            self.assertNotIn("\x1b", str((resp.result or {}).get("text") or ""))
        finally:
            cleanup()

    def test_terminal_snapshot_restores_main_screen_after_alternate_screen_exit(self) -> None:
        from unittest.mock import patch
        ...
        _, cleanup = self._with_home()
        try:
            group = create_group(load_registry(), title="terminal-snapshot-alt-screen")
            add_actor(group, actor_id="peer1", title="Peer 1", runtime="codex", runner="pty")
            ...
            self.assertIn("shell prompt", text)
            self.assertNotIn("vim buffer", text)
            self.assertEqual((resp.result or {}).get("end_cursor"), 56)
        finally:
            cleanup()
