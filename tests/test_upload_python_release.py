...
def test_existing_hashes_reads_registry_digests(monkeypatch) -> None:
    payload = {
        "releases": {
            "0.4.35rc1": [
                {
                    "filename": "cccc_pair-0.4.35rc1-py3-none-any.whl",
    ...
    monkeypatch.setattr(upload_python_release.urllib.request, "urlopen", urlopen)

    assert upload_python_release.existing_hashes("testpypi") == {
        "cccc_pair-0.4.35rc1-py3-none-any.whl": "abc123"
    }
    assert observed == 1
...
def test_same_filename_with_different_hash_fails_without_upload(monkeypatch, tmp_path: Path) -> None:
    ...
    assert upload_python_release.upload_missing("testpypi", [wheel]) == 1
