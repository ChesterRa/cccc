...
class TestFederationRouteLookup(unittest.TestCase):
    def test_resolves_active_trust_by_local_and_remote_group(self) -> None:
        from cccc.daemon.federation.route_lookup import resolve_remote_group_route
        from cccc.kernel.federation import pairing as pairing_kernel

        with tempfile.TemporaryDirectory() as td, patch("cccc.kernel.federation.pairing.ensure_home", return_value=Path(td)):
