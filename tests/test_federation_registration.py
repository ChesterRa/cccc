...
class TestFederationRegistration(unittest.TestCase):
    ...
    def test_normalize_url(self) -> None:
        from cccc.kernel.federation.registration import normalize_url
    ...
    def test_upsert_dedup_same_target(self) -> None:
        ...
        try:
            a = upsert_registration("g1", "https://hub.example/", credential_ref="cred-1")
            # Same group + URL that normalizes identically => upsert, not a new record.
            b = upsert_registration("g1", "HTTPS://hub.example", credential_ref="cred-2")
            self.assertEqual(a["registration_id"], b["registration_id"])
            self.assertEqual(len(list_registrations()), 1)
            self.assertEqual(b["credential_ref"], "cred-2")
    ...
    def test_distinct_group_or_url_distinct_record(self) -> None:
        ...
        _, cleanup = self._with_home()
        try:
            upsert_registration("g1", "https://hub.example/", credential_ref="cred-ref-1")
            upsert_registration("g2", "https://hub.example/", credential_ref="cred-ref-1")
            upsert_registration("g1", "https://other.example/", credential_ref="cred-ref-1")
            self.assertEqual(len(list_registrations()), 3)
        finally:
            cleanup()

    def test_get_and_delete(self) -> None:
        from cccc.kernel.federation.registration import (
            delete_registration,
            get_registration,
            upsert_registration,
        )
        ...
        _, cleanup = self._with_home()
        try:
            rec = upsert_registration("g1", "https://hub.example/", credential_ref="cred-ref-1")
    ...
    def test_rejects_token_shaped_credential_ref(self) -> None:
        ...
        try:
            ...
            path = home / "federation_registrations.yaml"
            if path.exists():
    ...
    def test_allows_empty_and_reference_shaped_credential_refs(self) -> None:
        ...
        try:
            ...
            legacy = upsert_registration("g1", "https://legacy.example/", credential_ref="cred-ref-1")
            self.assertEqual(legacy["credential_ref"], "cred-ref-1")
        finally:
            cleanup()
