...
### Fixed
- **Group run controls moved into the sidebar group menu.** Launch/resume/pause and stop now live in each Group's `⋮` menu (and its context menu), work for any Group rather than only the selected one, and no longer occupy the chat header. The same menu gains a Delete entry that reuses the Group settings confirmation.
...
- **Stopping CCCC no longer strands Claude Actors.** Managed Actors now stop concurrently instead of one after another, so a busy host finishes inside the forced-exit deadline; a forced exit (second Ctrl-C or timeout) still sends one stop request per Claude Agent View session, which is not an owned process tree and used to keep running; and the session receipt records the Agent View pty host pid and start time so that a resume against a vanished Agent View daemon reaps that host and its orphaned worker before starting a fresh one.
- **Voice Secretary no longer leaks memory while the workbench stays open.** V8 keeps one context per render for every variable a closure captures, and the composer control's memoized callbacks referenced each other across renders, so every render scope stayed reachable and pinned that moment's document list, chat events, and drafts (about 0.25 MB per Group switch, more while recording). Its callbacks now use identity-stable wrappers (`useStableCallback`) and the document list lives behind a ref, so old render scopes are released.
