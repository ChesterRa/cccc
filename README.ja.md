...
Claude Code、Codex、Gemini など 11 のランタイムをひとつの永続グループで。**
...
| **マルチランタイム編成** | Claude Code、Codex CLI、Grok Build、OpenCode、ChatGPT Web、Gemini CLI など 11 種の主要ランタイムを混在利用でき、さらに `custom` も扱える |
...
CCCC は 11 種の主要ランタイムでエージェントを編成し、残りは `custom` で扱えます。同一グループ内で各 actor が異なるランタイムを使用可能です。
